...
shinyServer(function(input, output, session) {
    ...
    dataset <- reactive({
      # query <- parseQueryString(session$clientData$url_search)
       ...
       # link <- query[["csvpath"]]
       # link <- "http://www.dcn.ed.ac.uk/camarades/example2.csv"
       # myfile <- getURL(link, ssl.verifyhost=F, ssl.verifypeer=F)
       # dataset <- read.csv(textConnection(myfile), header=T)
       dataset <- read.csv("D:/shinyapp/datafile/example2.csv", header = T)
    })
        ...
        for (j in 1:length(OutList)){ # within j-th outcome
          ...
          if (type == "SC"){
            ...
            for (k in 1:length(UniIntID)) { # within k-th Intervention type
              ...
              srec = stem[stem[,"InterventionID"] %in% UniIntID[k],]     # nrow(srec) = 1
              crec = ctem[ctem[,"InterventionID"] %in% UniIntID[k],]
              ...
              colnames(df) = c("Author", "Year", "OutcomeId", "ch.s","ch.c", "ch.t", "Mean.S","Mean.C","Mean.T",
                               ...
                               "ES","SE", colnames(dat[,(fix+1):ncol(dat)]))
